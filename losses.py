...
def gradientLoss_win(img1, img2, img3, window_size=12):
    size = window_size
    grad_sum = 0
    filter = torch.Tensor([[[[0, 1, 0], [1, -4, 1], [0, 1, 0]]]]).cuda()
    
    img1_grad_1 = F.conv2d(img1[:, 0:1, :, :], filter, padding=1)
    img1_grad_2 = F.conv2d(img1[:, 1:2, :, :], filter, padding=1)
    img1_grad_3 = F.conv2d(img1[:, 2:3, :, :], filter, padding=1)
    img1_grad = torch.cat((img1_grad_1, img1_grad_2, img1_grad_3), 1)
    
    img2_grad = F.conv2d(img2, filter, padding=1)
    
    img3_grad_1 = F.conv2d(img3[:, 0:1, :, :], filter, padding=1)
    img3_grad_2 = F.conv2d(img3[:, 1:2, :, :], filter, padding=1)
    img3_grad_3 = F.conv2d(img3[:, 2:3, :, :], filter, padding=1)
    img3_grad = torch.cat((img3_grad_1, img3_grad_2, img3_grad_3), 1)

    grad_sum = torch.mean(torch.pow(img1_grad - img3_grad, 2)) + torch.mean(torch.pow(img2_grad - torch.mean(img3_grad, 1, keepdim = True), 2))
    return grad_sum
...
class Fusionloss(nn.Module):
    ...
    def forward(self, vi, ir, fu, mask=None):
        ...
        #内容损失
        loss_in = F.l1_loss(vi, fu) + torch.mean((vi_gray < ir) * torch.pow((fu_gray - ir), 2))
        
        #梯度
        vi_grad_x, vi_grad_y = self.sobelconv(vi_gray)
